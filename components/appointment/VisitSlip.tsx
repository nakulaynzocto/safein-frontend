        ...
        <div ref={ref} className="print-only">
            ...
            <div className="flex flex-col items-center text-center space-y-3">
                ...
                <div className="border-b-2 border-dashed border-slate-300 w-full pb-3 mb-2">
                    <h2 className="text-lg font-semibold uppercase tracking-widest text-[#3882a5] leading-none mb-1">{companyName}</h2>
                    <h1 className="text-sm font-bold uppercase tracking-[4px] text-slate-500">Visit Pass</h1>
                    <p className="text-xs font-bold text-slate-400 tabular-nums mt-1">ID: {appointment._id?.slice(-8).toUpperCase()}</p>
                </div>
                ...
                <div className="w-full space-y-2 mt-4">
                    ...
                    <div className="text-center">
                        ...
                        <p className="text-lg font-semibold text-slate-800 uppercase leading-tight mt-1">{visitor?.name}</p>
                    ...
                    <div className="grid grid-cols-1 gap-3 py-4 border-y border-dashed border-slate-200 my-4">
                        <div className="text-center">
                            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest leading-none">Meeting To</p>
                            <p className="text-sm font-bold text-slate-800 uppercase mt-1">{employee?.name}</p>
                        </div>
                        <div className="text-center">
                            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest leading-none">Purpose</p>
                    ...
                    <div className="flex justify-between items-center bg-slate-50 p-2 rounded-lg border border-slate-100">
                        <div className="text-left">
                            <p className="text-xs font-bold text-slate-400 uppercase leading-none">Date</p>
                            <p className="text-xs font-bold text-slate-800 mt-1">
                                {details?.scheduledDate ? format(new Date(details.scheduledDate), "MMM dd, yyyy") : "---"}
                            </p>
                        </div>
                        <div className="text-right">
                            <p className="text-xs font-bold text-slate-400 uppercase leading-none">In Time</p>
                            <p className="text-xs font-bold text-slate-800 mt-1">{details?.scheduledTime || "---"}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="pt-6 w-full text-center">
                    <div className="border-t border-dashed border-slate-300 pt-4 mb-2">
                        <p className="text-xs font-bold text-slate-400 uppercase italic">Valid for today only</p>
                    </div>
