...
export function StatusPage({
  ...
  const renderButton = (action: { label: string; onClick: () => void; href?: string }, variant: "default" | "outline" = "default") => {
    if (action.href) {
      return (
    ...
    <div className={`min-h-screen flex items-center justify-center bg-gray-50 p-4 ${className}`}>
      <Card className="w-full max-w-md shadow-lg">
        <CardHeader className="text-center pb-4">
          ...
          <CardTitle className={`text-xl font-bold ${config.titleColor}`}>
            {title}
          </CardTitle>
        </CardHeader>
